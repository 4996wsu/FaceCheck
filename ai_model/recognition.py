...
def recognize_faces(frame, device, mtcnn, resnet, embedding_list, name_list):
    ...
    #img_cropped_list will have the cropped faces(face coordinates) from the image
    #prob_list will have the probability of the face being a face
    ...
    if img_cropped_list is not None:
        ...
        #bounding box will have the coordinates of the faces in the image
        
        boxes, _ = mtcnn.detect(img)
        # Iterate over the detected faces
        for i, prob in enumerate(prob_list):
            if prob > 0.90:  # If the face detection probability is high enough
                ...
                if min_dist < 0.9:
                    ...
                    # If the minimum distance is greater than the threshold, consider the face unrecognized
                ...
                # If probability is low, consider the face as not detected/reliable enough, so you can skip it
                #recognized_names.append("Unknown")
                continue
    # Return the recognized names and the frame with bounding boxes and names drawn on it
...
def update_attendance(recognized_names, class_section,date ,time):
    ...
    # Remove recognized students from the list of all students
